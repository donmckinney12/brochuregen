...
from services.analytics_service import (
    ...
    get_top_performers,
    track_engagement as _track_engagement,  # noqa: F401
)
...
async def track_engagement(
    request: TrackEngagementRequest, db: Session = Depends(get_db)
):
    """
    ...
    try:
        _track_engagement(
            db, request.brochure_id, request.section_id, request.duration_ms
        )
...
async def get_analytics(brochure_id: int, db: Session = Depends(get_db)):
    """
    ...
    engagements = (
        db.query(BrochureEngagement)
        .filter(BrochureEngagement.brochure_id == brochure_id)
        .all()
    )
