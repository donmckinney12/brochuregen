    ...
    if (brochures.length === 0) {
        return (
            <div className="bg-[var(--background)]/60 rounded-2xl border border-[var(--glass-border)] p-12 text-center border-dashed backdrop-blur-md">
                <div className="w-16 h-16 bg-[var(--foreground)]/5 rounded-full flex items-center justify-center mx-auto mb-4 text-[var(--foreground)]/80 border border-[var(--glass-border)]">
                ...
                <p className="text-[var(--foreground)]/80 mb-6 max-w-sm mx-auto text-sm">Synchronize with the network to generate your first document.</p>
            </div>
        );
    }
                ...
                <div key={item.id} className="premium-card overflow-hidden hover:shadow-[var(--accent-primary)]/10 transition-all group border-[var(--glass-border)]">
                    ...
                    <div className="p-6">
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-[var(--glass-border)]">
                            <span className="text-[10px] text-[var(--foreground)]/80 font-bold uppercase tracking-widest">
                                {new Date(item.created_at).toLocaleDateString()}
                            </span>
                            <div className="flex gap-1">
                                <button className="p-2 text-[var(--foreground)]/80 hover:text-[var(--accent-primary)] transition-colors">
                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                </button>
                                <button className="p-2 text-[var(--foreground)]/80 hover:text-[var(--accent-secondary)] transition-colors">
