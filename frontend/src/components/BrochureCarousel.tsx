    ...
    return (
        <div className="w-full max-w-7xl mx-auto py-16 px-4">
            <div className="text-center mb-12">
                <span className="text-[var(--accent-primary)] font-bold tracking-wider uppercase text-sm">Product Showcase</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 perspective-1000">
                {BROCHURES.map((brochure, index) => (
                    <div
                        key={brochure.id}
                        className={`group relative bg-[var(--glass-bg)] rounded-2xl shadow-xl overflow-hidden transform transition-all duration-500 hover:-translate-y-3 hover:shadow-2xl border border-[var(--glass-border)] ${index === 1 ? 'md:-mt-8' : ''}`}
                    >
                        {/* Image Container */}
                        <div className="h-64 overflow-hidden relative cursor-pointer" onClick={() => setSelectedBrochure(brochure)}>
                            ...
                            <div className="absolute top-4 right-4 z-20">
                                <span className="px-3 py-1 bg-[var(--background)]/90 dark:bg-slate-900/90 backdrop-blur-md rounded-full text-xs font-bold text-[var(--foreground)] shadow-sm">
                                    {brochure.category}
                                </span>
                            </div>
                        ...
                        <div className="p-6 relative">
                            <div className={`absolute -top-10 right-6 w-16 h-16 rounded-full bg-gradient-to-br ${brochure.color} border-4 border-[var(--background)] shadow-md flex items-center justify-center z-20 transition-colors`}>
                                <svg className="w-8 h-8 text-[var(--foreground)] opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                            </div>

                            <h3 className="text-xl font-bold text-[var(--foreground)] mb-2">{brochure.title}</h3>
                            <div className="space-y-2 mb-6">
                                <div className="h-2 w-full bg-[var(--foreground)]/5 rounded-full overflow-hidden">
                                    <div className="h-full bg-[var(--accent-primary)]/30 w-3/4"></div>
                                </div>
                                <div className="h-2 w-full bg-[var(--foreground)]/5 rounded-full overflow-hidden">
                                    <div className="h-full bg-[var(--accent-primary)]/30 w-1/2"></div>
                                </div>
                            </div>
            ...
            {selectedBrochure && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setSelectedBrochure(null)}>
                    <div
                        className="bg-[var(--background)] rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 relative flex flex-col md:flex-row border border-[var(--glass-border)]"
                    ...
                    >
                        ...
                        <div className="w-full md:w-2/3 h-64 md:h-auto bg-[var(--foreground)]/5 relative">
                            <img
                        ...
                        <div className="w-full md:w-1/3 p-8 flex flex-col justify-center">
                            ...
                            <button className="mt-8 w-full py-3 rounded-xl bg-[var(--accent-primary)] hover:opacity-90 text-white font-bold transition-all shadow-lg active:scale-95">
                                Use This Template
                            </button>
                        </div>
                    </div>
