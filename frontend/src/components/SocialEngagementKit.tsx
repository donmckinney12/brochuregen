...
interface SocialEngagementKitProps {
    posts: SocialPost[] | string;
    brochureId?: number;
}

export default function SocialEngagementKit({ posts, brochureId }: SocialEngagementKitProps) {
    ...
    return (
        <section className="py-12 sm:py-20">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h3 className="text-xl font-black text-[var(--foreground)] italic tracking-tighter uppercase">Social Engagement Kit</h3>
                    <p className="text-[10px] font-bold text-[var(--foreground)]/80 uppercase tracking-widest mt-1 italic">AI-Generated Multi-Channel Promotion Assets</p>
                </div>
            </div>
                ...
                {postList.map((post, i) => {
                    ...
                    return (
                        <motion.div
                            ...
                            className="group relative p-6 sm:p-12 bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-[1.5rem] sm:rounded-[2rem] hover:border-[var(--accent-primary)]/30 transition-all duration-500 overflow-hidden shadow-xl"
                        >
                            ...
                            <div className="relative z-10 flex items-center justify-between mb-6">
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    <button
                                        ...
                                        disabled={publishingIndex === i || publishedIndices.includes(i)}
                                        className={`flex-1 sm:flex-none p-2 sm:p-2.5 rounded-xl border transition-all flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest ${publishedIndices.includes(i) ? 'bg-blue-500 text-white border-blue-500' : 'bg-[var(--accent-primary)] text-white border-[var(--accent-primary)] hover:opacity-90 disabled:opacity-50'}`}
                                    >
                                        {publishingIndex === i ? <Loader2 size={12} className="animate-spin" /> : publishedIndices.includes(i) ? <Check size={12} /> : <Send size={12} />}
                                        <span>{publishedIndices.includes(i) ? 'Published' : 'Publish'}</span>
                                    </button>
                                </div>
                            </div>
