        ...
        <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-[var(--glass-bg)] backdrop-blur-xl border-b border-[var(--glass-border)] py-3 shadow-lg' : 'bg-transparent py-6'}`}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
                ...
                <div className="hidden md:flex items-center justify-center gap-10">
                    {['Features', 'Pricing', 'Use-Cases'].map((item) => (
                        <Link
                            ...
                            className={`text-[11px] font-black uppercase tracking-[0.3em] hover:text-[var(--accent-primary)] transition-all ${pathname === `/${item.toLowerCase()}` ? 'text-[var(--accent-primary)] font-black' : 'text-[var(--foreground)]/80'}`}
                        >
                            {item}
                        </Link>
                    ))}
                ...
                <div className="flex items-center gap-2 sm:gap-4">
                    ...
                    <button
                        onClick={toggleTheme}
                        className="flex p-2 rounded-full hover:bg-[var(--foreground)]/10 transition-colors text-[var(--foreground)]/80"
                    ...
                    <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="md:hidden p-2 text-[var(--foreground)]/80 hover:bg-[var(--foreground)]/10 rounded-lg">
                        {isMenuOpen ? (
            ...
            {isMenuOpen && (
                <div className="md:hidden absolute top-full left-0 w-full bg-[var(--background)] border-b border-[var(--glass-border)] animate-in slide-in-from-top-2 shadow-2xl">
                    <div className="px-6 py-8 flex flex-col gap-6">
                        {['Features', 'Pricing', 'Use-Cases'].map((item) => (
                            <Link key={item} href={`/${item.toLowerCase()}`} onClick={() => setIsMenuOpen(false)} className="text-[10px] font-black uppercase tracking-[0.3em] text-[var(--foreground)]/80 hover:text-[var(--accent-primary)]">
                                {item}
                            </Link>
                        ))}
                        {mounted && (
