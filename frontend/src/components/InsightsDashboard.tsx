    ...
    return (
        <div className="space-y-10 pb-20 animate-in fade-in slide-in-from-bottom-8 duration-1000">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    { label: 'Brochure Created', value: totalBrochures, icon: <Activity className="w-5 h-5" />, color: 'text-indigo-400' },
                    { label: 'Total Views', value: totalViews, icon: <Zap className="w-5 h-5" />, color: 'text-cyan-400' },
                    { label: 'Visitors', value: totalVisitors, icon: <Shield className="w-5 h-5" />, color: 'text-emerald-400' },
                    { label: 'Estimated Value', value: `$${(totalBrochures * 150).toLocaleString()}`, icon: <Target className="w-5 h-5" />, color: 'text-amber-400' }
                ].map((stat, i) => (
                    <motion.div key={i} whileHover={{ y: -5 }} className="premium-card p-6">
                        <div className="flex items-center gap-3 mb-4">
                            <div className={`${stat.color} opacity-80`}>{stat.icon}</div>
                            <span className="text-[10px] font-black uppercase tracking-widest opacity-60 text-[var(--foreground)]">{stat.label}</span>
                        </div>
            ...
            <div className="premium-card p-8">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-xl font-black italic uppercase tracking-tighter text-[var(--foreground)]">Usage Growth</h2>
                    <div className="flex gap-2 p-1 bg-[var(--foreground)]/10 rounded-xl border border-[var(--glass-border)]">
                        {['generation', 'visitors'].map((type) => (
                            <button key={type} onClick={() => setChartType(type as any)} className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${chartType === type ? 'bg-[var(--accent-primary)] text-white shadow-lg' : 'text-[var(--foreground)]/40 hover:text-[var(--foreground)]/60'}`}>
                                {type === 'generation' ? 'Brochures' : 'Visitors'}
                            </button>
                        ))}
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* Variant Performance */}
                <div className="premium-card p-8">
                    <h3 className="text-[10px] font-black uppercase tracking-[0.3em] opacity-60 text-[var(--foreground)] mb-8">Performance Optimization</h3>
                ...
                {/* Resonance Heatmap */}
                <div className="premium-card p-8 border-[var(--accent-primary)]/10 bg-[var(--accent-primary)]/[0.02]">
                    <div className="flex justify-between items-center mb-8">
                        <h3 className="text-[10px] font-black uppercase tracking-[0.3em] opacity-60 text-[var(--foreground)]">Resonance Heatmap</h3>
                        ...
                        <div className="h-64">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart layout="vertical" data={engagementData.sort((a, b) => b.total_hover_time - a.total_hover_time)}>
                                    ...
                                    <YAxis dataKey="section_id" type="category" width={80} tick={{ fontSize: 8, fill: 'currentColor', opacity: 0.6 }} axisLine={false} tickLine={false} />
                                    <Tooltip content={({ active, payload }) => {
                                        if (active && payload?.length) {
                                            const d = payload[0].payload;
                                            return <div className="bg-[var(--glass-bg)] backdrop-blur-3xl border border-[var(--glass-border)] p-3 rounded-xl shadow-2xl">
                                                <p className="text-[8px] font-black uppercase opacity-60 text-[var(--foreground)] mb-1">{d.section_id}</p>
                                                <div className="text-xs font-black text-[var(--foreground)]">{(d.total_hover_time / 1000).toFixed(1)}s Attention</div>
                                            </div>;
                                        }
                                        return null;
                                    }} />
            ...
            {/* Activity Stream */}
            <div className="premium-card p-8">
                <h3 className="text-xl font-black italic uppercase tracking-tighter text-[var(--foreground)] mb-8">Studio Stream</h3>
                <div className="space-y-4">
                    {activities.slice(0, 5).map((activity) => (
                        <div key={activity.id} className="flex items-center justify-between p-4 bg-[var(--foreground)]/5 rounded-2xl border border-[var(--glass-border)]">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h4 className="text-sm font-bold text-[var(--foreground)]">{activity.details}</h4>
                                    <p className="text-[10px] font-black uppercase opacity-30 text-[var(--foreground)]">{activity.action}</p>
                                </div>
                            </div>
                            <span className="text-[10px] font-black opacity-20 text-[var(--foreground)]">{new Date(activity.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                        </div>
                    ))}
                </div>
            </div>
            ...
            {/* Feedback Stream */}
            <div className="premium-card p-8 border-[var(--accent-secondary)]/10 bg-[var(--accent-secondary)]/[0.02]">
                <h3 className="text-xl font-black italic uppercase tracking-tighter text-[var(--foreground)] mb-8">Client Feedback</h3>
                <div className="space-y-4">
                    {feedback.length === 0 ? (
                        <div className="h-32 flex items-center justify-center opacity-40 text-xs font-bold uppercase tracking-widest text-[var(--foreground)]">No feedback detected yet.</div>
                    ) : (
                        feedback.map((fb) => (
                            <div key={fb.id} className="p-5 bg-[var(--foreground)]/[0.03] rounded-2xl border border-[var(--glass-border)] hover:border-[var(--accent-secondary)]/30 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div>
                                    <h4 className="text-sm font-bold italic leading-relaxed text-[var(--foreground)]/90">"{fb.text}"</h4>
                                    <div className="flex items-center gap-3 mt-2">
                                        <span className="text-[9px] font-black text-[var(--accent-secondary)] uppercase tracking-widest border border-[var(--accent-secondary)]/20 px-2 py-0.5 rounded-full bg-[var(--accent-secondary)]/10">
                                            {fb.section_id.replace('_', ' ')}
                                        </span>
                                        <span className="text-[10px] font-bold uppercase text-[var(--foreground)]/40 tracking-widest">
            ...
            {funnel && (
                <div className="premium-card p-6 sm:p-8">
                    <h3 className="text-lg font-black italic uppercase tracking-tighter flex items-center gap-3 text-[var(--foreground)] mb-8">
                            ...
                            <div key={b.id} className="flex items-center gap-4 p-4 bg-white/5 border border-white/5 rounded-2xl hover:border-white/10 transition-all">
                                ...
                                <div className="flex gap-6 shrink-0">
                                    <div className="text-center">
                                        <p className="text-lg font-black text-[var(--foreground)]">{b.views}</p>
                                        <p className="text-[8px] font-black uppercase tracking-widest opacity-30 text-[var(--foreground)]">Views</p>
                                    </div>
                                    <div className="text-center">
            ...
            {/* CTA */}
            <div className="premium-card p-12 border-[var(--accent-primary)]/10 bg-gradient-to-br from-[var(--accent-primary)]/[0.05] to-[var(--accent-secondary)]/[0.05] flex flex-col items-center text-center">
                <Target className="w-12 h-12 text-indigo-400 mb-6" />
                <h3 className="text-2xl font-black italic uppercase tracking-tighter mb-2 text-[var(--foreground)]">Scale Your Reach</h3>
                <p className="text-sm opacity-40 mb-8 max-w-sm text-[var(--foreground)]">Consistent brand messaging increases conversion by 42%. Scale your presence now.</p>
                <button onClick={onOpenVault} className="px-10 py-4 bg-[var(--foreground)] text-[var(--background)] font-black text-xs uppercase tracking-widest rounded-2xl hover:scale-105 transition-all shadow-xl">
                    Access Brand Vault
                </button>
            </div>
        </div>
