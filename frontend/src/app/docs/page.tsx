...
export default function HelpCenter() {
    ...
    const sections = [
        ...
        { id: 'cli-reference', title: 'CLI Reference', icon: '💻' },
        { id: 'api-integration', title: 'API Integration', icon: '🔌' },
        { id: 'security', title: 'Security', icon: '🛡️' },
    ];

    const protocolCommands = [
        { cmd: 'brochuregen initiate --url <target>', desc: 'Start a new generation from a target URL.' },
        { cmd: 'brochuregen edit --id <uuid> --tone <vibe>', desc: 'Adjust the brand voice of a generated asset.' },
        { cmd: 'brochuregen export --format pdf', desc: 'Generate a print-ready PDF document.' },
    ];
            ...
            <main className="pt-32 pb-20 max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-4 gap-12">
                ...
                <aside className="lg:col-span-1 border-r border-slate-100 dark:border-slate-800 pr-8 hidden lg:block">
                    <div className="sticky top-40 space-y-2">
                        <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-6 italic">Guide</h3>
                        {sections.map((sec) => (
                            <button
                                key={sec.id}
                ...
                <div className="lg:col-span-3">
                    <header className="mb-16">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/10 text-cyan-400 text-[10px] font-black mb-6 border border-cyan-500/20 uppercase tracking-widest">
                            Official Documentation v3.0.4
                        </div>
                        <h1 className="text-5xl font-black tracking-tighter mb-8 italic uppercase">Systems <span className="gradient-text">Guide</span></h1>
                        <p className="text-xl text-slate-600 dark:text-slate-400 leading-relaxed font-light">
                            Welcome to the Documentation. This guide contains everything you need to start generating professional brochures.
                        </p>
                    </header>
                    ...
                    <div className="space-y-12">
                        {activeSection === 'getting-started' && (
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6" transition={{ duration: 0.3 }}>
                                <h2 className="text-2xl font-black uppercase tracking-tight">01. Getting Started</h2>
                                <p className="text-slate-400 leading-relaxed italic">Begin by entering a website URL. Our AI will analyze the content and generate high-fidelity marketing assets in seconds.</p>
                                <div className="p-6 bg-white/5 border border-white/10 rounded-2xl">
                                    <h4 className="font-bold text-cyan-400 mb-2 uppercase text-xs tracking-widest">Setup Steps</h4>
                                    <ol className="list-decimal list-inside text-sm text-slate-300 space-y-2">
                                        <li>Register your account.</li>
                                        <li>Configure your brand profile.</li>
                                        <li>Enter a target URL for AI analysis.</li>
                                    </ol>
                                </div>
                            </motion.div>
                        )}
                        ...
                        {activeSection === 'core-concepts' && (
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6" transition={{ duration: 0.3 }}>
                                <h2 className="text-2xl font-black uppercase tracking-tight">AI Content Generation</h2>
                                <p className="text-slate-400 leading-relaxed italic">Our system utilizes a high-performance infrastructure to analyze, synthesize, and render brand-accurate collateral.</p>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="p-4 bg-slate-900 border border-white/10 rounded-xl">
                                        <div className="text-cyan-500 mb-2 text-xl">⚡</div>
                                        <div className="font-bold text-xs uppercase mb-1 text-white">Content Analysis Layer</div>
                                        <div className="text-[10px] text-slate-500 italic">Advanced website traversal and asset extraction.</div>
                                    </div>
                                    <div className="p-4 bg-slate-900 border border-white/10 rounded-xl">
                                        <div className="text-fuchsia-500 mb-2 text-xl">💡</div>
                                        <div className="font-bold text-xs uppercase mb-1 text-white">AI Engine</div>
                                        <div className="text-[10px] text-slate-500 italic">Advanced copywriting and layout analysis.</div>
                                    </div>
                                </div>
                            </motion.div>
                        )}
                        ...
                        {activeSection === 'api-integration' && (
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6" transition={{ duration: 0.3 }}>
                                <h2 className="text-2xl font-black uppercase tracking-tight">API Integration</h2>
                                <p className="text-slate-400 leading-relaxed italic">Connect your external applications directly to the BrochureGen API using our standard REST endpoints.</p>
                                <div className="bg-slate-950 p-6 rounded-2xl border border-white/5 font-mono text-[10px]">
                                    <div className="flex gap-2 mb-2">
                                        ...
                                        <span className="text-white/40">/api/v1/generate</span>
                                    </div>
                                    <div className="text-white">
                                        {`{`} <br />
                                        &nbsp;&nbsp;&quot;url&quot;: &quot;https://example.com&quot;, <br />
                                        &nbsp;&nbsp;&quot;layout_theme&quot;: &quot;modern&quot; <br />
                                        {`}`}
                                    </div>
                                </div>
                            </motion.div>
                        ...
                        {activeSection === 'security' && (
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6" transition={{ duration: 0.3 }}>
                                ...
                                <p className="text-slate-400 leading-relaxed italic">All data transmitted through our systems is protected by industry-standard AES-256 encryption.</p>
                                <ul className="space-y-4">
                                    {[
                                        { t: 'Encryption', d: 'Data at rest and in transit uses bank-grade security protocols.' },
                                        { t: 'Isolation', d: 'Each workspace operates in a secure, isolated environment.' },
                                        { t: 'Compliance', d: 'GDPR ready institutional-grade infrastructure.' }
                                    ].map((item, i) => (
                    ...
                    <section className="mt-32 pt-16 border-t border-white/5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">
                            {[
                                { q: "Usage Limits?", a: "Each tier has a defined credit allocation. Credits reset every 30 days." },
                                { q: "Data Protection?", a: "Your brand library is encrypted and stored in a secure cloud infrastructure." },
                                { q: "Custom Branding?", a: "Enterprise users can request custom training and branding requirements." }
                            ].map((faq, i) => (
